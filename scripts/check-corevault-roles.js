...
const { createPublicClient, http, isAddress, keccak256, stringToHex } = require('viem');
const fs = require('fs');
...
if (!coreVault) {
  console.error('❌ Missing CoreVault address. Provide --coreVault 0x... or set CORE_VAULT_ADDRESS in env.');
  process.exit(1);
}

if (!orderBook) {
  console.error('❌ Missing OrderBook address. Provide --orderBook 0x... or set ORDERBOOK_ADDRESS in env.');
  process.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    console.log(`ORDERBOOK_ROLE  on CoreVault for OrderBook: ${okOrderBook ? '✅ PRESENT' : '❌ MISSING'}`);
    console.log(`SETTLEMENT_ROLE on CoreVault for OrderBook: ${okSettlement ? '✅ PRESENT' : '❌ MISSING'}`);

    console.log('\nDiagnosis:');
    if (!okSettlement) {
      console.log('- Missing SETTLEMENT_ROLE will cause updateMarkPrice() to revert, breaking pokeLiquidations().');
    }
    if (!okOrderBook) {
      console.log('- Missing ORDERBOOK_ROLE will prevent position updates during liquidation/trade execution.');
    }
    ...
    if (!okOrderBook || !okSettlement) {
      ...
      console.log(`cast send ${coreVault} "grantRole(bytes32,address)" ${okOrderBook ? SETTLEMENT_ROLE : ORDERBOOK_ROLE} ${orderBook} --rpc-url ${rpcUrl} --private-key $ADMIN_PK`);
    ...
    console.error('\n❌ Error while checking roles:', e?.shortMessage || e?.message || String(e));
    process.exit(1);
  }
}
