...
async function main() {
  ...
  const hasRoleBefore = await vaultRouter.hasRole(ORDERBOOK_ROLE, ORDERBOOK_ADDRESS);
  ...
  const isAdmin = await vaultRouter.hasRole(DEFAULT_ADMIN_ROLE, deployer.address);
  ...
  const hasRoleAfter = await vaultRouter.hasRole(ORDERBOOK_ROLE, ORDERBOOK_ADDRESS);
  console.log("📋 OrderBook has ORDERBOOK_ROLE (after):", hasRoleAfter);

  if (hasRoleAfter) {
    console.log("\n🎉 SUCCESS! OrderBook now has ORDERBOOK_ROLE");
    console.log("🔧 This should fix the TradingRouter getMultiMarketPrices errors");
  } else {
