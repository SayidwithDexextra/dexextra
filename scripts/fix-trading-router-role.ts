...
async function fixTradingRouterRole() {
  ...
  // This would require the admin private key - for demonstration purposes
  ...
  try {
    ...
    console.log('🔧 This is the cause of the AccessControlUnauthorizedAccount error.');
    
    console.log('\n📋 To fix this issue, an admin needs to run:');
    console.log('');
    console.log('```javascript');
    console.log(`// Grant ORDERBOOK_ROLE to TradingRouter`);
    console.log(`const vaultRouter = await ethers.getContractAt("VaultRouter", "${CONTRACT_ADDRESSES.vaultRouter}");`);
    console.log(`const ORDERBOOK_ROLE = await vaultRouter.ORDERBOOK_ROLE();`);
    console.log(`await vaultRouter.grantRole(ORDERBOOK_ROLE, "${CONTRACT_ADDRESSES.tradingRouter}");`);
    console.log('```');
...
async function main() {
  ...
  if (result.success) {
    if (result.alreadyGranted) {
      ...
      console.log('🔧 SOLUTION: Grant ORDERBOOK_ROLE to TradingRouter in VaultRouter');
      console.log('🎯 IMPACT: This will fix the placeLimitOrder AccessControlUnauthorizedAccount error');
    }
  } else {
