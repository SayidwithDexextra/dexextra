...
if (!orderBook || !isAddress(orderBook)) {
  console.error(
    "❌ Provide a valid --orderBook 0x... address or set ORDERBOOK_ADDRESS in env"
  );
  process.exit(1);
}
...
const OBLiqEvents = [
  ...
  {
    type: "event",
    name: "LiquidationRecursionGuardSet",
    inputs: [{ indexed: false, name: "inProgress", type: "bool" }],
  },
  {
    type: "event",
    name: "LiquidationTraderBeingChecked",
    inputs: [
  ...
  {
    type: "event",
    name: "LiquidationLiquidatableCheck",
    inputs: [
      { indexed: true, name: "trader", type: "address" },
  ...
  {
    type: "event",
    name: "LiquidationLiquidityCheck",
    inputs: [
  ...
  {
    type: "event",
    name: "LiquidationMarketOrderResult",
    inputs: [
      { indexed: true, name: "trader", type: "address" },
...
async function main() {
  ...
  const marketId =
    marketStatic && Array.isArray(marketStatic) && marketStatic.length >= 2
      ? marketStatic[1]
      : marketIdTry;

  const vaultAddr =
    marketStatic && Array.isArray(marketStatic) && marketStatic.length >= 1
      ? marketStatic[0]
  ...
  const fromBlock =
    latest > BigInt(blockWindow) ? latest - BigInt(blockWindow) : 0n;
  ...
  if (!logs || logs.length === 0) {
    ...
    for (const log of logs) {
      ...
      if (name === "LiquidationCheckStarted") {
        summary.started++;
        const { markPrice, tradersLength, startIndex, endIndex } =
          log.args || {};
        console.log(
          `Start: mark=${markPrice} traders=${tradersLength} range=[${startIndex},${endIndex}) @${log.blockNumber}`
        );
      } else if (name === "LiquidationRecursionGuardSet") {
        if (log.args?.inProgress) summary.recursionGuardTrue++;
        else summary.recursionGuardFalse++;
        ...
        if (success) summary.liqSuccess++;
        else summary.liqNone++;
        console.log(
          `Result: success=${success} reason=${
            reason
        ...
        summary.finished++;
        const { tradersChecked, liquidationsTriggered, nextStartIndex } =
          log.args || {};
        console.log(
          `Finish: checked=${tradersChecked} triggered=${liquidationsTriggered} next=${nextStartIndex} @${log.blockNumber}`
        );
      }
    }
  ...
  if (activeNow.length && users && Array.isArray(users)) {
    const missing = activeNow.filter(
      (u) => !users.some((x) => x && x.toLowerCase() === u)
    );
    if (missing.length) {
      console.log(
