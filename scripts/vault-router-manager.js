...
async function main() {
  ...
  try {
    ...
    const currentCollateral = await vaultRouter.userCollateral(
      deployer.address
    );
    const availableCollateral = await vaultRouter.getAvailableCollateral(
      deployer.address
    );
    ...
    console.log(
      `💰 MockUSDC Balance: ${hre.ethers.formatUnits(usdcBalance, 6)} USDC`
    );
    console.log(
      `🏦 Total Collateral in Vault: ${hre.ethers.formatUnits(
        currentCollateral,
        6
      )} USDC`
    );
    console.log(
      `✅ Available for Trading: ${hre.ethers.formatUnits(
        availableCollateral,
        6
      )} USDC`
    );
    console.log(
      `📝 Current Allowance: ${hre.ethers.formatUnits(allowance, 6)} USDC\n`
    );
    ...
    if (usdcBalance === 0n) {
      ...
      try {
        ...
        console.log(
          `💰 New MockUSDC Balance: ${hre.ethers.formatUnits(
            newBalance,
            6
          )} USDC\n`
        );
    ...
    if (currentUsdcBalance > 0n && allowance < currentUsdcBalance) {
      ...
      console.log(
        `🚀 Approving ${hre.ethers.formatUnits(approveAmount, 6)} USDC...`
      );
    ...
    const finalAvailable = await vaultRouter.getAvailableCollateral(
      deployer.address
    );
    const marginSummary = await vaultRouter.getMarginSummary(deployer.address);

    console.log(
      `🏦 Total Collateral: ${hre.ethers.formatUnits(finalCollateral, 6)} USDC`
    );
    console.log(
      `✅ Available for Trading: ${hre.ethers.formatUnits(
        finalAvailable,
    ...
    console.log(
      `📊 Margin Used: ${hre.ethers.formatUnits(
        marginSummary.marginUsed,
        6
      )} USDC`
    );
    console.log(
      `📊 Margin Reserved: ${hre.ethers.formatUnits(
    ...
    if (finalAvailable >= requiredMargin) {
      ...
      console.log(
        `✅ Required: ${hre.ethers.formatUnits(requiredMargin, 6)} USDC`
      );
      console.log(
        `✅ Available: ${hre.ethers.formatUnits(finalAvailable, 6)} USDC`
      );
      ...
      console.log(
        `❌ Need: ${hre.ethers.formatUnits(
          requiredMargin - finalAvailable,
          6
        )} more USDC`
      );
    }
