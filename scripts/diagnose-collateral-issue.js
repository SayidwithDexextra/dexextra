...
async function main() {
  ...
  console.log(
    `   Same address?: ${
      deployer.address.toLowerCase() === ERROR_WALLET.toLowerCase()
    }\n`
  );
  ...
  try {
    ...
    const deployerCollateral = await vaultRouter.userCollateral(
      deployer.address
    );
    const deployerAvailable = await vaultRouter.getAvailableCollateral(
      deployer.address
    );

    console.log(
      `MockUSDC Balance: ${hre.ethers.formatUnits(deployerUsdcBalance, 6)} USDC`
    );
    console.log(
      `Total Collateral: ${hre.ethers.formatUnits(deployerCollateral, 6)} USDC`
    );
    console.log(
    ...
    const errorAvailable = await vaultRouter.getAvailableCollateral(
      ERROR_WALLET
    );
    const errorAllowance = await mockUSDC.allowance(
      ERROR_WALLET,
      VAULT_ROUTER_ADDRESS
    );

    console.log(
      `MockUSDC Balance: ${hre.ethers.formatUnits(errorUsdcBalance, 6)} USDC`
    );
    console.log(
      `Total Collateral: ${hre.ethers.formatUnits(errorCollateral, 6)} USDC`
    );
    console.log(
      `Available Collateral: ${hre.ethers.formatUnits(errorAvailable, 6)} USDC`
    );
    console.log(
      `VaultRouter Allowance: ${hre.ethers.formatUnits(
        errorAllowance,
        6
      )} USDC\n`
    );
    ...
    if (deployer.address.toLowerCase() !== ERROR_WALLET.toLowerCase()) {
      ...
      console.log(
        "   The script deposited collateral to the deployer account,"
      );
      console.log("   but the UI is using a different wallet address.");
      console.log("");
      console.log("🔧 SOLUTIONS:");
      console.log(
        "   1. Use the same wallet in the UI as the deployer account"
      );
      ...
      if (errorCollateral === 0n) {
        console.log(
          "❌ Collateral is 0 - deposit transaction might have failed"
        );
    ...
    console.log(
      `Required for trade: ${hre.ethers.formatUnits(requiredMargin, 6)} USDC`
    );
    ...
    if (errorAvailable >= requiredMargin) {
      ...
      console.log(
        `   Need to deposit: ${hre.ethers.formatUnits(needed, 6)} USDC`
      );
    }
  } catch (error) {
