...
async function main() {
  ...
  try {
    ...
    try {
      await tradingRouter.placeLimitOrder.staticCall(
        marketId,
        side,
        size,
        price
      ...
      if (staticError.message.includes("insufficient collateral")) {
        ...
        const userCollateral = await vaultRouter.userCollateral(
          deployer.address
        );
        const marginUsed = await vaultRouter.getTotalMarginUsed(
          deployer.address
        );
        ...
        console.log(
          `   User Collateral: ${hre.ethers.formatUnits(
            userCollateral,
            6
          )} USDC`
        );
        console.log(
          `   Margin Used: ${hre.ethers.formatUnits(marginUsed, 6)} USDC`
        );
        console.log(
        ...
        console.log(
          `   Calculated Available: ${hre.ethers.formatUnits(
            available,
            6
          )} USDC`
        );
        ...
        const isMarketAuthorized = await vaultRouter.authorizedMarkets(
          marketId
        );
    ...
    console.log(
      "   This suggests the issue is with the frontend state management"
    );
    ...
    if (error.message.includes("insufficient collateral")) {
      ...
      const available = await vaultRouter.getAvailableCollateral(
        deployer.address
      );

      console.log(
        `   Direct userCollateral: ${hre.ethers.formatUnits(
          collateral,
          6
        )} USDC`
      );
      console.log(
        `   getAvailableCollateral: ${hre.ethers.formatUnits(
          available,
          6
        )} USDC`
      );
