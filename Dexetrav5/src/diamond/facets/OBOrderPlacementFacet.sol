...
import "../libraries/OrderBookMatchingLib.sol";
...
contract OBOrderPlacementFacet {
    ...
    using OrderBookMatchingLib for OrderBookStorage.State;
    ...
    function placeMarginMarketOrder(uint256 amount, bool isBuy) external marketActive returns (uint256 filledAmount) {
        ...
        require(available >= estMargin, "lc");
    ...
    modifier onlySelf() {
        require(msg.sender == address(this), "so");
        _;
    }
    ...
    function cancelOrderBy(address trader, uint256 orderId) external onlySelf {
        ...
        require(order.trader != address(0) && order.trader == trader, "OB");
        if (order.isBuy) {
        ...
        returns (uint256 newOrderId)
    {
        ...
        require(oldOrder.trader != address(0), "no");
        require(oldOrder.trader == trader, "nw");
        ...
        if (isMarginOrder) {
            marginRequired = s.calculateMarginRequired( amount, price, isBuy);
        }
    ...
    function cancelOrder(uint256 orderId) external {
        OrderBookStorage.State storage s = OrderBookStorage.state();
        OrderBookStorage.Order storage order = s.orders[orderId];
        require(order.trader != address(0), "no");
        require(order.trader == msg.sender, "nw");
        if (order.isBuy) {
            _removeFromBuyBook(s, orderId, order.price);
        } else {
            _removeFromSellBook(s, orderId, order.price);
        ...
        returns (uint256 newOrderId)
    {
        ...
        OrderBookStorage.Order storage oldOrder = s.orders[orderId];
        require(oldOrder.trader != address(0), "no");
        require(oldOrder.trader == msg.sender, "nw");
    ...
    // --- Internal matching with batched execution ---
    
    uint256 private constant MAX_MATCHES_PER_ORDER = 50;
        ...
        returns (uint256)
    {
            ...
            while (remaining > 0 && currentOrderId != 0 && matchCount < MAX_MATCHES_PER_ORDER) {
                ...
                if (sellOrder.isMarginOrder) {
                    if (sellOrder.amount == 0) {
                        ...
                        uint256 newReserved = s.calculateMarginRequired( sellOrder.amount, sellOrder.price, sellOrder.isBuy);
                ...
                if (sellOrder.amount == 0) {
                    ...
                    s.removeOrderFromUserList( sellOrder.trader, currentOrderId);
                    delete s.orders[currentOrderId];
                }
        ...
        if (matchCount > 0) {
            OrderBookStorage.PendingMatch[] memory trimmedMatches = new OrderBookStorage.PendingMatch[](matchCount);
            for (uint256 i = 0; i < matchCount; i++) {
                trimmedMatches[i] = pendingMatches[i];
            }
            IOBTradeExecutionFacet(address(this)).obExecuteTradeBatch(
                buyOrder.trader,
                true,
            ...
            while (remaining > 0 && currentOrderId != 0 && matchCount < MAX_MATCHES_PER_ORDER) {
                ...
                if (buyOrder.isMarginOrder) {
                    if (buyOrder.amount == 0) {
                        ...
                        uint256 newReserved = s.calculateMarginRequired( buyOrder.amount, buyOrder.price, buyOrder.isBuy);
                ...
                if (buyOrder.amount == 0) {
                    ...
                    s.removeOrderFromUserList( buyOrder.trader, currentOrderId);
                    delete s.orders[currentOrderId];
                }
    ...
    function _addToBuyBook(OrderBookStorage.State storage s, uint256 orderId, uint256 price, uint256 amount) private {
        if (!s.buyLevels[price].exists) {
            ...
            s.orders[lastOrderId].nextOrderId = orderId;
            s.orders[orderId].prevOrderId = lastOrderId;
    ...
    function _removeOrderFromLevel(OrderBookStorage.State storage s, uint256 orderId, uint256 price, bool isBuy) private {
        s.removeOrderFromLevel(orderId, price, isBuy);
        if (s.buyLevels[price].totalAmount == 0 && isBuy) { _prunePriceLevel(s, price, isBuy); }
        if (s.sellLevels[price].totalAmount == 0 && !isBuy) { _prunePriceLevel(s, price, isBuy); }
    }
    ...
    function _removeFromBuyBook(OrderBookStorage.State storage s, uint256 orderId, uint256 price) private {
        ...
        if (price == s.bestBid && !s.buyLevels[price].exists) { s.bestBid = s.findNewBestBid(); }
    }
    ...
    function _removeFromSellBook(OrderBookStorage.State storage s, uint256 orderId, uint256 price) private {
        ...
        if (price == s.bestAsk && !s.sellLevels[price].exists) { s.bestAsk = s.findNewBestAsk(); }
    }
    ...
    function _marginRequiredForMarketOrder(
        ...
        if (isReducing) {
            ...
            return s.calculateMarginRequired( amount - absCurrentSize, price, isBuy);
        }

        return s.calculateMarginRequired( amount, price, isBuy);
    }
    ...
    function _placeLimitOrder(
        ...
        if (remaining > 0) {
            ...
            if (isMarginOrder) {
                uint256 adjustedReserved = s.calculateMarginRequired( remaining, price, isBuy);
            ...
            s.addOrderToUserList(trader, orderId);
    ...
    /// @dev Deferred liquidation check - only triggers every N operations or when price moves significantly
    uint256 private constant LIQUIDATION_CHECK_INTERVAL = 10;
    uint256 private constant PRICE_CHANGE_THRESHOLD_BPS = 50; // 0.5% price change triggers check
    ...
    function _onOrderBookLiquidityChanged() private {
        ...
        s.liquidationDirtyCount++;
        
        bool shouldCheck = false;
        uint256 currentMark = s.lastMarkPrice;
        
        if (s.liquidationDirtyCount >= LIQUIDATION_CHECK_INTERVAL) {
            shouldCheck = true;
        }
        
        if (!shouldCheck && currentMark > 0 && s.lastCachedMarkPrice > 0) {
            uint256 priceDiff = currentMark > s.lastCachedMarkPrice 
                ? currentMark - s.lastCachedMarkPrice 
                : s.lastCachedMarkPrice - currentMark;
            uint256 threshold = (s.lastCachedMarkPrice * PRICE_CHANGE_THRESHOLD_BPS) / 10000;
            if (priceDiff >= threshold) {
                shouldCheck = true;
            }
        }
        
        if (!shouldCheck && block.number > s.lastLiquidationTriggerBlock + 5) {
            shouldCheck = true;
        }
        
        if (shouldCheck && gasleft() > 100_000 && currentMark > 0) {
            s.liquidationDirtyCount = 0;
