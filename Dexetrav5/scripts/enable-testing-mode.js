...
async function main() {
  ...
  const enabled = String(process.env.ENABLED || getArg("--enabled", "true")).toLowerCase() !== "false";
  console.log("OrderBook:", ob, "-> enableTestingMode(", enabled, ")");
  const lifecycle = await ethers.getContractAt("MarketLifecycleFacet", ob);
  const tx = await lifecycle.enableTestingMode(enabled);
  console.log("tx:", tx.hash);
  await tx.wait();
  console.log("✅ testingMode =", enabled);
}
