...
// start-settlement-window.js
// Calls MarketLifecycleFacet.startSettlementChallengeWindow() on a target OrderBook (Diamond)
//
// Usage:
//   HARDHAT_NETWORK=hyperliquid ORDERBOOK=0x... npx hardhat run scripts/start-settlement-window.js --network hyperliquid
//   Or:
//   npx hardhat run scripts/start-settlement-window.js --network hyperliquid -- --orderbook 0x...
...
async function main() {
  console.log("\n⏳ Start Settlement Challenge Window");
  ...
  console.log("🟢 Calling startSettlementChallengeWindow...");
  try {
    const tx = await lifecycle.startSettlementChallengeWindow();
    console.log("   tx:", tx.hash);
    const rc = await tx.wait();
    console.log("✅ Settlement challenge window started at block", rc.blockNumber);
  } catch (e) {
    console.error("❌ Failed to start settlement challenge window:", extractError(e));
    process.exit(1);
  }
}
...
main()
  .then(() => process.exit(0))
  .catch((e) => {
    console.error("\n❌ start-settlement-window failed:", extractError(e));
    process.exit(1);
  });
