...
async function main() {
  ...
  const admin = adminEnv && ethers.isAddress(adminEnv) ? adminEnv : deployer.address;
  const feeRecipient =
    feeRecipientEnv && ethers.isAddress(feeRecipientEnv) ? feeRecipientEnv : admin;
  ...
  console.log(`EIP712_FACTORY_DOMAIN_NAME=DexetraFactory`);
