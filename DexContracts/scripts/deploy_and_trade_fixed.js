...
async function main() {
  console.log("🚀 Complete vAMM Deployment & Trading Demo (Fixed)");
  ...
  try {
    ...
    const initialSupply = 1000000; // 1 million tokens (will be multiplied by decimals in constructor)
    ...
    // Mint additional USDC for trading (MockUSDC has 6 decimals)
    const additionalMintAmount = hre.ethers.parseUnits("10000", 6); // 10,000 USDC
    console.log(
      "   🔨 Minting additional",
      hre.ethers.formatUnits(additionalMintAmount, 6),
      "USDC..."
    );
    await mockUSDC.mint(deployer.address, additionalMintAmount);

    const totalBalance = await mockUSDC.balanceOf(deployer.address);
    console.log(
      "   📊 Total USDC Balance:",
      hre.ethers.formatUnits(totalBalance, 6),
      "USDC"
    );

    // Approve vault to spend USDC
    const approvalAmount = hre.ethers.parseUnits("8000", 6); // 8,000 USDC
    console.log(
    ...
    console.log(
      "   📊 Vault USDC Balance:",
      hre.ethers.formatUnits(vaultBalance, 6),
      "USDC"
    );
    ...
    const collateral1 = hre.ethers.parseUnits("100", 18); // $100 collateral (18 decimals for USD)
    const leverage1 = 5;
    const minPrice = 0;
    ...
    console.log(
      "\n🎯 Pro Tip: The bonding curve creates pump.fund-style behavior:"
    );
    console.log("   • Early buyers get massive price pumps with small amounts");
    console.log("   • Later buyers face progressively higher costs");
    console.log(
      "   • This incentivizes early adoption and creates viral growth"
    );
    ...
    console.error("\n🔍 Debug Information:");
    console.error("- Ensure you have sufficient MATIC for gas fees");
    console.error(
      "- Check that all contract deployments completed successfully"
    );
    console.error("- Verify USDC balance and allowances");
