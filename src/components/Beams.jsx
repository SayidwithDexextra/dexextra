...
// Cache compiled materials to avoid recompilation
...
function getBeamMaterial(speed, noiseIntensity, scale) {
  const cacheKey = `${speed}-${noiseIntensity}-${scale}`;
...
const Beams = ({
  ...
  useEffect(() => {
    const id = requestIdleCallback?.(() => setIsReady(true)) ?? setTimeout(() => setIsReady(true), 0);
    return () => {
  ...
  const beamMaterial = useMemo(
    () => getBeamMaterial(speed, noiseIntensity, scale),
    [speed, noiseIntensity, scale]
  );

  if (!isReady) {
    // Placeholder to avoid layout shift
    return <div className="beams-container" style={{ background: '#000000' }} />;
  }
...
// Component to continuously invalidate frames when using demand frameloop
...
function createStackedPlanesBufferGeometry(n, width, height, spacing, heightSegments) {
  const cacheKey = `${n}-${width}-${height}-${spacing}-${heightSegments}`;
...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  // Reduced from 100 to 50 segments - still looks smooth but halves vertex count
