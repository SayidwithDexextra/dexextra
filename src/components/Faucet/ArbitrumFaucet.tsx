...
export function ArbitrumFaucet({ className = '' }: ArbitrumFaucetProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-3 md:p-2.5 border-b border-[#1A1A1A]">
        <div className="flex items-center gap-2">
          <div className="w-1.5 h-1.5 rounded-full flex-shrink-0 bg-blue-400" />
          <span className="text-xs md:text-[11px] font-medium text-white">Arbitrum Token Claim</span>
        </div>
        <div className="flex items-center gap-2 text-[11px] md:text-[10px] text-[#808080]">
      ...
      <div className="p-3 md:p-2.5 space-y-3">
        ...
        {tokenAddress ? (
          <div className="text-[11px] md:text-[10px] text-[#606060] flex items-center gap-2">
          ...
          <div className="text-[11px] md:text-[10px] text-[#ff9f1c]">
            Set `SPOKE_ARBITRUM_USDC_ADDRESS` to enable claiming on Arbitrum.
          </div>
        )}

        <div className="space-y-2 md:space-y-1.5">
          <div className="flex items-center gap-2 text-[11px] md:text-[10px] text-[#9CA3AF]">
          ...
          <input
            ...
            className="w-full bg-[#0A0A0A] border border-[#222222] rounded px-3 py-2.5 md:py-2 text-sm md:text-[12px] text-white outline-none focus:border-[#3B82F6] transition-colors"
          />
          <div className="text-[10px] md:text-[9px] text-[#606060]">
        ...
        {status.error && (
          <div className="text-[11px] md:text-[10px] text-[#f87171] bg-[#1A0F0F] border border-[#3b1a1a] rounded px-3 py-2.5 md:py-2">
            {status.error}
          </div>
        )}
        {status.success && (
          <div className="text-[11px] md:text-[10px] text-[#34d399] bg-[#0f1714] border border-[#1f2f29] rounded px-3 py-2.5 md:py-2">
            {status.success}
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2">
          {!isConnected ? (
            <button
              ...
              className="w-full md:w-auto px-4 py-2.5 md:px-3 md:py-2 text-xs md:text-[11px] rounded border border-[#222222] text-white bg-[#111111] hover:bg-[#1d1d1d] active:bg-[#252525] disabled:opacity-50 transition-colors"
            >
            ...
            <button
              ...
              className="w-full md:w-auto px-4 py-2.5 md:px-3 md:py-2 text-xs md:text-[11px] rounded border border-[#3B82F6] text-white bg-[#0F172A] hover:bg-[#111827] active:bg-[#1e293b] transition-colors"
            >
              Switch to Arbitrum
            </button>
          ) : (
            <button
              onClick={handleClaim}
              disabled={!canClaim}
              className="w-full md:w-auto px-4 py-2.5 md:px-3 md:py-2 text-xs md:text-[11px] rounded border border-[#22c55e] text-white bg-[#0f1f16] hover:bg-[#13291d] active:bg-[#1a3326] disabled:opacity-50 flex items-center justify-center gap-2 transition-colors"
            >
              {isClaiming ? (
                <>
