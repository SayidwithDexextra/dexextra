...
type TabId = 'overview' | 'trades' | 'fees' | 'revenue' | 'markets' | 'pnl' | 'settlements' | 'activity'
            ...
            {activeTab === 'overview' && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  <ActivityChart data={filteredDailyData} isLoading={isLoading} />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  <FeesByMarketChart data={feesByMarket} isLoading={isLoading} />
                  <CumulativePnlChart 
...
function AccountHero({
  ...
  // Use on-chain P&L values (same source as Portfolio sidebar)
      ...
      <div className="px-6 pb-5">
        ...
        <div className="flex items-center gap-6 mt-4 pt-4 border-t border-[var(--t-stroke-sub,#1a1a1a)]">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[13px] font-mono font-medium ${isProfit ? 'text-[#4ade80]' : 'text-[#f87171]'}`}>
              {isLoading ? '—' : formatCurrency(profitAfterFees, { showSign: true })}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[13px] font-mono ${realizedPnl >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]'}`}>
              {isLoading ? '—' : formatCurrency(realizedPnl, { showSign: true })}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-mono text-[#fbbf24]">
              {isLoading ? '—' : formatCurrency(summary.totalFeesPaid)}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-mono text-[#4ade80]">
              {isLoading ? '—' : formatCurrency(summary.totalDeposits)}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-mono text-[var(--t-fg-sub,#909090)]">
              {isLoading ? '—' : formatCurrency(summary.totalWithdrawals)}
            </span>
          </div>
        </div>
      </div>
...
function MarketsTab({
  ...
  const netPnl = useMemo(() => {
    // TODO: Calculate actual ROI once we have revenue data per market
    return 0
  }, [])

  if (createdMarkets.length === 0 && !isLoading) {
    return (
      <div className="bg-[#0A0A0A] rounded border border-[#141414] p-6 text-center">
        <div className="w-12 h-12 rounded-full bg-[#141414] flex items-center justify-center mx-auto mb-3">
          <svg className="w-6 h-6 text-[#505050]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <p className="text-[12px] text-[#707070]">No markets created yet</p>
        <p className="text-[11px] text-[#505050] mt-1">Create your first market to see it here</p>
      </div>
    )
  }
