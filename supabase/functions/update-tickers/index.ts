...
const ON_CHAIN_DEFAULT_PRICE = 1_000_000; // 1.0 USDC — Solidity fallback when book is empty
...
async function readMarkPrice(address: string): Promise<number | null> {
  console.log("[update-tickers] readMarkPrice start", { address });
  try {
    ...
    console.log("[update-tickers] readMarkPrice success", { address, price });
    return price;
  } catch (err: any) {
...
async function getExistingMarkPrice(marketId: string): Promise<number | null> {
  try {
...
async function upsertTickerAndSeries(
  marketId: string,
  price: number | null,
  isoTs: string
) {
  ...
  if (price != null && price > 0) {
    // When on-chain returns the empty-book default (1 USDC), don't overwrite
    // a more meaningful existing price — the initial startPrice from market
    // creation is more accurate than the Solidity fallback.
    if (price === ON_CHAIN_DEFAULT_PRICE) {
      const existing = await getExistingMarkPrice(marketId);
      if (existing != null && existing > ON_CHAIN_DEFAULT_PRICE) {
        console.log(
          "[update-tickers] skipping default price — existing price is more meaningful",
          { marketId, onChainPrice: price, existingPrice: existing }
        );
        return;
      }
    }
    ...
    console.log("[update-tickers] rpc upsert_market_ticker done", { marketId });
    ...
    console.log("[update-tickers] rpc insert_market_price done", { marketId });
  } else {
    console.log("[update-tickers] rpc upsert_market_ticker", {
      marketId,
      price: 0,
  ...
  console.log("[update-tickers] upsertTickerAndSeries done", { marketId });
}
...
function collectAddresses(obj: any, out = new Set<string>()): Set<string> {
  if (!obj) return out;
...
async function verifyAlchemySignature(
  ...
  if (!ALCHEMY_SIGNING_KEY) {
    console.log(
      "[update-tickers] verifyAlchemySignature no signing key configured"
    );
    return { valid: true, reason: "no signing key configured" };
  }
  const header =
  ...
  if (!header) {
    console.warn(
      "[update-tickers] verifyAlchemySignature missing signature header"
    );
  ...
  const key = await crypto.subtle.importKey(
    "raw",
    enc.encode(ALCHEMY_SIGNING_KEY),
  ...
  const candidateRaw = header.includes("=")
    ? header.split("=").pop()!.trim()
    : header.trim();
  const candidateLower = candidateRaw.toLowerCase();
  const matches =
    candidateLower === expectedHex || candidateRaw === expectedB64;
  console.log("[update-tickers] verifyAlchemySignature result", {
    valid: matches,
  });
  return { valid: matches, reason: matches ? "ok" : "mismatch" };
}

function isAuthorizedCron(req: Request): boolean {
  if (!ENABLE_POLLING) {
    console.warn(
      "[update-tickers] isAuthorizedCron disabled: polling not enabled"
    );
    return false;
  }
  if (!CRON_SECRET) {
    console.warn(
      "[update-tickers] isAuthorizedCron disabled: cron secret not configured"
    );
    return false;
  }
...
async function handleWebhook(req: Request): Promise<Response> {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ ok: true, ignored: true, reason: "bad json" }),
      { status: 200, headers: { "content-type": "application/json" } }
    );
  }
  const addresses = Array.from(collectAddresses(payload));
  console.log("[update-tickers] Addresses", addresses);
  console.log(
    "[update-tickers] Addresses list:",
  ...
  if (addresses.length === 0) {
    ...
    return new Response(
      JSON.stringify({ ok: true, updated: 0, reason: "no addresses" }),
      { status: 200, headers: { "content-type": "application/json" } }
    );
  }
  const toMatch = addresses;
  const orFilter = toMatch
    .map((a) => `market_address.ilike.${a}`)
    .join(",");
  ...
  return new Response(
    JSON.stringify({ ok: true, updated, matched: matched?.length || 0 }),
    { status: 200, headers: { "content-type": "application/json" } }
  );
}
  ...
  try {
    ...
    if (req.method === "GET") {
      if (isAuthorizedCron(req)) {
        console.log("[update-tickers] authorized cron poll", { traceId });
        return await handlePoll();
      }
      console.log("[update-tickers] health check", { traceId });
      return new Response(
        JSON.stringify({
          ok: true,
