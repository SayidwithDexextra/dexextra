...
contract SimpleVAMM is ISimpleVAMM {
    ...
    // ULTRA-SENSITIVE PRICE DISCOVERY - Extremely small reserves for maximum sensitivity
    uint256 public baseVirtualBaseReserves = 1 * PRICE_PRECISION; // 10,000x smaller than legacy - extreme sensitivity
    uint256 public baseVirtualQuoteReserves = 1 * PRICE_PRECISION; // 10,000x smaller than legacy - extreme sensitivity
    ...
    /**
     * @dev Calculates ultra-sensitive virtual reserves based on net position
     * Traditional futures: net long position = price up, net short = price down
     * REMOVED dynamic scaling for maximum price sensitivity
     */
    function getEffectiveReserves() public view returns (uint256 baseReserves, uint256 quoteReserves) {
        // Use fixed minimal reserves for MAXIMUM sensitivity - no dynamic scaling!
        baseReserves = baseVirtualBaseReserves;
        quoteReserves = baseVirtualQuoteReserves;
        ...
        if (netPosition > 0) {
            ...
            // SCALING FIX: Convert USDC units (6 decimals) to ETH units (18 decimals)
            uint256 impact = uint256(netPosition) * 1e12; // Scale up by 1e12 to match ETH units
    ...
    function getPriceImpact(uint256 size, bool isLong) external view override returns (uint256) {
        ...
        if (isLong) {
            ...
            // SCALING FIX: Convert USDC units to ETH units
            uint256 impact = size * 1e12; // Scale up by 1e12 to match ETH units
