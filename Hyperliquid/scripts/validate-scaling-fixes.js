...
async function main() {
  ...
  console.log("Got signers:", deployer.address, user1.address, user2.address);
  ...
  console.log("✅ VaultRouter deployed:", vaultRouter.address);

  // Deploy OrderBookFactoryMinimal
  const OrderBookFactoryMinimal = await ethers.getContractFactory(
    "OrderBookFactoryMinimal"
  );
  const factory = await OrderBookFactoryMinimal.deploy(
    vaultRouter.address,
    deployerAddress
  );
  ...
  console.log("✅ TradingRouter deployed:", tradingRouter.address);

  // Create a market
  console.log("\n📊 Creating market...");
  const marketId = ethers.id("ETH_USD_TEST");
  await factory.createTraditionalMarket("ETH/USD", {
  ...
  await vaultRouter.grantRole(ORDERBOOK_ROLE, orderBook.address);
  ...
  try {
    ...
    console.log(
      "   MARGIN_PERCENTAGE:",
      MARGIN_PERCENTAGE.toString(),
      "(expected: 10)"
    );
    console.log(
      "   MAX_REASONABLE_PRICE:",
      MAX_REASONABLE_PRICE.toString(),
      "(expected: 1000000000)"
    );
    console.log(
      "   MIN_REASONABLE_PRICE:",
      MIN_REASONABLE_PRICE.toString(),
      "(expected: 10000)"
    );
    console.log(
      "   MAX_ORDER_SIZE:",
      MAX_ORDER_SIZE.toString(),
      "(expected: 1000000000000)"
    );

    if (
      PRICE_PRECISION.toString() === "1000000" &&
  ...
  await mockUSDC.connect(user1).approve(vaultRouter.address, depositAmount);
  ...
  try {
    ...
    const marginReserved = await vaultRouter.getTotalMarginReserved(
      user1Address
    );
    const expectedMargin = ethers.parseUnits("50", 6); // $50

    console.log(
      "   Actual margin reserved:",
      ethers.formatUnits(marginReserved, 6),
      "USDC"
    );
    console.log(
      "   Expected margin:",
      ethers.formatUnits(expectedMargin, 6),
      "USDC"
    );
  ...
  try {
    ...
    if (
      error.message.includes("insufficient collateral") &&
  ...
  console.log("\n📋 Summary:");
  console.log(
    "✅ Precision constants properly set for USDC 6-decimal compatibility"
  );
