...
class PolygonContractVerifier {
  ...
  async loadDeploymentData() {
    ...
    if (this.config.deploymentFile) {
      ...
      if (fs.existsSync(this.config.deploymentFile)) {
        this.deploymentData = JSON.parse(
          fs.readFileSync(this.config.deploymentFile, "utf8")
        );
        console.log(
          "   ✅ Loaded deployment data from:",
          this.config.deploymentFile
        );
      } else {
        throw new Error(
          `Deployment file not found: ${this.config.deploymentFile}`
        );
      }
    } else {
      ...
      const files = fs
        .readdirSync(".")
        .filter((file) => file.startsWith("deployments-polygon-updated-"))
        .sort()
        .reverse();

      if (files.length === 0) {
        throw new Error(
          "No deployment files found. Please run deployment first."
        );
      }
      ...
      console.log(
        "   ✅ Auto-loaded latest deployment file:",
        this.config.deploymentFile
      );
    }
  ...
  prepareVerificationList() {
    ...
    // OrderBook contracts
    Object.entries(addresses.orderBooks).forEach(
      ([marketName, orderBookAddress]) => {
        this.contractsToVerify.push({
          name: "OrderBook",
    ...
    console.log(
      `   📊 Found ${this.contractsToVerify.length} contracts to process`
    );
    console.log(
      `   ✅ Will verify ${
        this.contractsToVerify.filter((c) => !c.skipVerification).length
      } contracts`
    );
    console.log(
      `   ⏭️  Will skip ${
  ...
  async verifyAllContracts() {
    ...
    if (failCount > 0) {
      ...
      console.log(
        "   You can retry verification later using the verify-contracts script."
      );
    }
  }
  ...
  async verifyContract(contract) {
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        console.log(
          `   🔄 Verification attempt ${attempt}/${this.config.retryAttempts}...`
        );
        ...
        console.log(
          `   🔗 View on Polygonscan: https://polygonscan.com/address/${contract.address}#code`
        );
        return true;
      } catch (error) {
        ...
        if (attempt === this.config.retryAttempts) {
          console.log(
            `   ❌ ${contract.name} verification failed after ${this.config.retryAttempts} attempts:`
          );
          console.log(`      Error: ${error.message}`);
          console.log(
            `   🔗 Contract address: https://polygonscan.com/address/${contract.address}`
          );
          return false;
        }
        ...
        console.log(
          `   ⏰ Waiting ${this.config.delayBetweenRetries}ms before retry...`
        );
        await new Promise((resolve) =>
          setTimeout(resolve, this.config.delayBetweenRetries)
        );
      }
    }
  ...
  async saveVerificationResults() {
    ...
    const verificationData = {
      ...
      contracts: this.contractsToVerify.map((contract) => ({
        ...
        polygonscanUrl: `https://polygonscan.com/address/${contract.address}${
          contract.skipVerification ? "" : "#code"
        }`,
    ...
    fs.writeFileSync(
      this.config.deploymentFile,
      JSON.stringify(this.deploymentData, null, 2)
    );
  ...
  printVerificationSummary() {
    ...
    this.contractsToVerify.forEach((contract) => {
      ...
      const url = `https://polygonscan.com/address/${contract.address}${
        contract.skipVerification ? "" : "#code"
      }`;
...
module.exports = { PolygonContractVerifier };
