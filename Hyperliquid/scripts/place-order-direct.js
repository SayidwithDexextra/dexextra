...
async function main() {
  ...
  try {
    ...
    const availableAfter = await vaultRouter.getAvailableCollateral(deployer.address);
    ...
    console.log(`💰 Remaining Available: ${hre.ethers.formatUnits(availableAfter, 6)} USDC`);
    ...
    if (error.message.includes("insufficient collateral")) {
      ...
      console.log("   This suggests the issue might be deeper than TradingRouter");
