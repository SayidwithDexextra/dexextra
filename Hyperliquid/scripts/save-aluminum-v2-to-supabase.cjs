...
/**
 * Save ALUMINUM_V2 market to Supabase using the existing schema
...
async function main() {
  ...
  // Use the standard schema from the existing deployment scripts
  const marketData = {
    metric_id: "ALUMINUM_V2", // Our display name
    description:
  ...
  try {
    ...
    if (existing && !checkError) {
      ...
      if (existing.market_address === aluminumV2OrderBook) {
        console.log("✅ Market addresses match - updating existing record");
        ...
        console.log(
          "⚠️  Different market address found. This might be a different deployment."
        );
        console.log("   Existing:", existing.market_address);
        console.log("   New:", aluminumV2OrderBook);
